...
!function ($) {
  ...
  * ========================= */
  ...
  , getActiveIndex: function () {
      this.$active = this.$element.find(this.options.itemname+'.active')
  ...
  , slide: function (type, next) {
      var $active = this.$element.find(this.options.itemname+'.active')
      ...
      $next = $next.length ? $next : this.$element.find(this.options.itemname)[fallback]()
			  ...
			  modalBG.unbind('click.modalEvent');
				...
				if(!locked) {
					...
					if(options.animation == "fadeAndPop") {
						modal.css({'display': 'block','opacity' : 0, 'visibility' : 'visible'});
